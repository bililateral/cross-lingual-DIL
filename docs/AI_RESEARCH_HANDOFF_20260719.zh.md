...
> **2026-08-28 权威交接替代声明**：本文保留为 2026-07-19 历史记录，不再代表当前科研状态。当前事实基础必须改读 `docs/AI_RESEARCH_HANDOFF_20260823.zh.md`，并优先阅读其中列出的 V9.4 模型可见捷径门合同与单次授权合同。当前正式中文合成数据、1,004 世界方法资格根、审核真值解封和 M0／M1／M2／M3 结果仍全部为零；V9.4 单次授权第一阶段已完成网页端精确复核及聚焦、相邻、全仓回归，但尚未创建时间密钥、授权政策或运行前置门。不得按本文较早的 Step27 主线恢复实验。
