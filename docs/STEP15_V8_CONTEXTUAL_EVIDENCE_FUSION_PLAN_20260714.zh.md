...
runner 的执行顺序固定为：freeze/check-only 与 41 项契约测试；检查 formal runtime 是完全不存在还是完整且哈希一致；首次运行仅重建中文 identifier-redacted E5 cache，并原子重建中英文 v7 feature tables；随后验证完整 runtime chain，再执行 v8 preflight、score-blind context queue snapshot、BGE/LaBSE/reranker 去标识特征、B0–B3 OOF bridge、evidence expert、Step12、return-sync manifest 与 Step20 gate。GPU 只用于 embedding/reranker；LR/linear ranker、evidence expert 和 Step12 主要使用 CPU。V8 preflight 将“CSV 列不存在”“整列在 train 上无可用值”和“可由预注册 fold-train median 插补的局部空值”分开处理；只有最后一种允许继续，且会输出逐特征缺失计数。Stage 8 读取 bridge 时以 artifact 中的全精度验证阈值为权威值，同时核验预测 CSV 的唯一 12 位阈值、summary 的 6 位展示值以及混淆矩阵；summary 展示精度不再被误当作模型阈值精度。Step12 的 readiness 计数使用独立可单测函数，Stage 8-10 的全部函数另受未绑定全局名称静态检查约束。
