...
`2026-07-16` work started on isolated branch `method/step21-synthetic-train-only-augmentation` after external proof-positive collection remained blocked. Step21 does not fabricate new benchmark truth. It adds deterministic, identifier-redacted Chinese positive-pair text augmentation under `reports/step21_synthetic_train_only/v1_20260716/`, with parent-pair/component lineage, fixed per-parent effective-weight budgets and a mandatory equal-effective-weight duplication control. It uses canonical Chinese train membership and the Step16I-v2 recomputed leak-free seller components rather than the superseded v7 representative-validation overlay. The primary track has `16` non-silver parent pairs (`1` direct-identifier and `15` style/structural soft); the physically separate silver direct/component sensitivity track has `85` parent pairs. Expected generated counts are `48` and `170`, but both tracks record `new_real_positive_count=0` and `new_independent_identity_count=0`. Method comparison uses five-fold seller-component grouped OOF on train only and never uses current valid/test for selection. Linux entry point: `scripts/run_step21_synthetic_train_only_linux_20260716.sh`; detailed protocol: `docs/STEP21_SYNTHETIC_TRAIN_ONLY_AUGMENTATION_20260716.zh.md`.
