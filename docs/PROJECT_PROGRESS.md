...
- a real frozen-input quota test shows that the original context-review pool supplies only `12` public-noise controls compatible with canonical valid, versus the fixed minimum of `20`. Two score/split-blind review rounds considered `11` proposed profile-URL pairs: source-literal validation removed one invalid supposedly shared `.onion` premise, leaving a final ten-pair candidate universe. Eight pairs received two-reviewer high-confidence agreement as public/victim-data URL negatives; two uncertain/disagreed pairs were excluded. The accepted eight plus the existing twelve exactly meet the valid quota. They remain evidence-expert controls only, never Step5 gold, primary supervision or benchmark rows;
...
The immediate next action is one clean Linux v3 run. Only v3 train-OOF selection and the corrected canonical-valid Step12 gates can decide method promotion. Step20 and Step11/17 remain blocked until promotion; the 200-row internal test remains diagnostic.
