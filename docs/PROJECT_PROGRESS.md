...
- the formal freeze has closed output/self hashes, 23 explicitly hashed direct/transitive inputs, `model_scores_read=false`, `thresholds_lowered=false`, a reproducible `--check-only` result and `36/36` passing v8 contract tests. Contracts cover exact control isolation, mutually exclusive readiness slices, fixed-test identity, component disjointness, transitive provenance, English-train-only counting, and the distinction between an absent v7 column, an imputable missing cell and a feature missing on every train row;
