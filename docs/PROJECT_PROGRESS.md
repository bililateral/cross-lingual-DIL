...
`2026-07-16` Step23 item-level multi-instance verification is implemented on isolated branch `method/step23-item-level-multi-instance` after Step21/22 synthetic augmentation failed. Step23 creates no new seller, item or identity label. It selects at most `12` real identifier-redacted train items per seller using seller-local exact-content deduplication and category round-robin, encodes only train sellers with frozen Multilingual-E5, and summarizes each real seller pair's item-to-item cosine distribution, bidirectional nearest-neighbor structure, mutual top-1 coverage, redacted exact overlap, category overlap and best-match style gaps. The read-only capacity boundary is English `401 pair / 582 seller / 62,075 raw item` and Chinese `573 pair / 676 seller / 3,439 raw item`; the maximum encoded cache is approximately `15,096` items. Aggregate redacted-E5 cosine, item-only and aggregate-plus-item representations share the same LR/L2, factorized evidence weights and five Chinese seller-component folds; separate source-only controls use English labels only. Selection is Chinese-train grouped OOF only, with a `>=0.02 AP` gain gate, component-bootstrap noninferiority and template/topic negative-score constraints. Current valid/test and Step16I Dev2 are not model-selection inputs. Numerical execution remains Linux-only; detailed design: `docs/STEP23_ITEM_LEVEL_MULTI_INSTANCE_VERIFICATION_20260716.zh.md`.
