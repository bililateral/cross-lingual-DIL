...
`2026-07-15` Step16-v8 readiness expansion is complete on branch `method/step15-v8-validation-slice-expansion`:

- formal freeze: `reports/step16_v8_validation_refreeze/readiness_expansion_v2_20260715/`;
- two independent agent reviews plus third-reviewer adjudication resolved the public-noise queue; a separate two-reviewer audit accepted the hash-bound cross-snapshot direct/component controls. This must be disclosed as agent-assisted evidence review, not human annotation;
- 20 train and 20 valid agent-reviewed high-confidence-silver public-noise controls were selected; 30 train/20 valid direct controls and 10 train/15 valid component controls were selected under global component-safe SHA256 ranking;
- all 115 selected expansion rows are isolated evidence-expert controls. Public-noise rows establish public/product-context identifier noise rather than absolute different-controller gold truth; direct/component rows are cross-snapshot same-account controls, not newly discovered alias pairs. Every expansion row has `usable_for_supervision=0`, `usable_for_core_transfer=0`, `primary_identity_model_eligible=0`, `benchmark_eligible=0` and can only train/validate the occurrence evidence expert;
- final state-backed readiness is valid public/direct/component `24/23/15` and train `20/30/10`, meeting the preregistered `20/20/15` and `20/30/10` requirements without lowering a threshold;
- primary split counts are train `924` (`401` English source-train plus `523` Chinese primary-train), valid `170`, and fixed internal test `200`; English valid/test rows do not enter v8 training; isolated evidence controls are train `60`, valid `55`;
- the 200 fixed test pair UIDs are byte-for-byte identical to the frozen v7 assignment and retain hash `ea9e5f46b742cb017e3122b00536e7741208adebb5d293ff31237e634d226ef5`; seller split overlap is `0`;
- Step4 and canonical Step7 pair universes both contain `3,964` rows. All pre-existing `3,857` rows retain their original fields. Six old `uncertain`/non-supervision rows receive negative review decisions only inside the evidence-control overlay; no usable binary supervision was overwritten or promoted into the primary benchmark;
- the formal freeze has closed output/self hashes, 23 explicitly hashed direct/transitive inputs, `model_scores_read=false`, `thresholds_lowered=false`, a reproducible `--check-only` result and `41/41` passing v8 contract tests. Contracts cover exact control isolation, mutually exclusive readiness slices, fixed-test identity, component disjointness, transitive provenance, English-train-only counting, the distinction between an absent v7 column, an imputable missing cell and a feature missing on every train row, exact bridge-threshold binding across artifact/CSV/summary precision layers, Step12 readiness-count execution, and unbound-global scanning across Stages 8-10;
- the v2 generated policy fixes atomic Linux publication: only the changed Chinese identifier-redacted E5 cache is rebuilt, while English and Chinese v7 feature tables are republished together under one output directory;
- the complete Linux entry point is `scripts/run_step15_v8_readiness_linux_20260715.sh`. Its formal policy paths cannot be redirected by environment variables; it fails on partial or hash-mismatched runtime bundles, verifies the full refreeze/cache/feature chain, and reuses a complete immutable runtime when only `V8_RUN_ID` changes. Linux numerical encoding/training has not run yet, so readiness completion is not a Step12 promotion claim;
- the first Linux preflight exposed a contract bug: `price_median_percentile_gap_abs` existed in the v7 table but contained legitimate empty cells, while preflight incorrectly treated any empty cell as an absent field despite the preregistered `train_median_per_feature` rule. Preflight now rejects truly absent columns and all-train-missing features, permits only cells covered by the frozen train-median contract, verifies the same B1 transform used by the bridge, and reports per-feature non-finite counts. No split, label, feature value or model hyperparameter changed;
- the first Stage 8 resume exposed a second serialization-contract bug: bridge artifacts retained the authoritative full-precision validation threshold, prediction CSVs retained 12 decimals, and generic metric summaries displayed 6 decimals, but the contextual-evidence consumer rounded the already-quantized CSV value and required exact equality with the summary. A half-way rounding case could therefore reject an internally consistent bridge. Stage 8 now binds the raw artifact threshold to the unique 12-decimal CSV token, verifies the 6-decimal summary directly from the raw artifact value, and requires persisted validation decisions to reproduce the summary confusion matrix. The bridge producer and Stages 1-7 remain unchanged, so an existing completed bridge can be reused without false provenance; only the failed `.contextual_evidence.incomplete` staging directory must be discarded or moved before resuming;
- the first Step12-v8 execution then exposed an unneeded `Counter` wrapper around an already materialized slice-count dictionary, without importing `Counter`. The wrapper has been replaced by a tested `validation_slice_readiness` function using plain dictionaries. A bytecode-level regression now scans every function in the Stage 8 consumer, Step12, sync-manifest builder and downstream gate for unresolved `LOAD_GLOBAL`/`LOAD_NAME` references, so this class of runtime-only missing import is no longer left to Linux execution;
- local regression status after both runtime-contract fixes is `41/41` v8 contracts and `90/90` combined v6/v7/v8 contracts. The fixes change no labels, split, score, selected representation, model coefficient, threshold or metric; they only correct cross-artifact validation and Step12 readiness-count execution;
- detailed protocol: `docs/STEP16_V8_READINESS_EXPANSION_PROTOCOL_20260715.zh.md`; method design: `docs/STEP15_V8_CONTEXTUAL_EVIDENCE_FUSION_PLAN_20260714.zh.md`.

The immediate next action is the clean Linux v8 run. Only its train-OOF selection and representative-valid Step12 gates can decide method promotion. The current 200-row test remains diagnostic and Step20 is still required for a publication claim.
