...
`2026-09-02` 当前正在实现英文初始化微调 V1。基模仍为与 M0 共用的本地 LaBSE；先只用 401 条真实英文训练对、582 个卖家、229 个连通分量和 26,114 条卖家—文本映射端到端训练编码器，不读取旧英文验证／测试标签。英文每轮覆盖全部文本，八轮约 232 次参数更新。随后在完整 500 世界中文训练集上，以完全相同的十轮训练日程比较通用初始化和英文监督初始化两个 `legacy18 + LaBSE6` 基础模型，中文分类头均重新初始化，身份三十三维暂不进入，避免遮住来源贡献。政策、Linux 训练器、无正式监督的图形处理器冒烟入口和 5 项聚焦合同测试已经实现；本地 5／5 通过，尚未运行 Linux 冒烟或数值训练，审核甲乙真值读取仍为零。
