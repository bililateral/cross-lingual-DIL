...
`2026-07-24` the current Step 7 → Step 28 meaning of M0 is now fixed explicitly. M0 is the complete English source-domain alias classifier trained and selected only with real English alias labels, then frozen to provide Step 28 base scores. It is not an encoder name and does not have to show an independent encoder gain over `legacy18`; `legacy18`-only, encoder-only and fused pipelines remain eligible. In Step 28, M0 is the frozen source model, M1 adds a matched module with no new identity-causal information, and M2 adds an identity-transfer module trained only on synthetic Chinese identity training data. Step 7-v4.1 has not yet selected M0 because complete-pipeline rankings are unstable between the clone-rich train data, de-cloned sensitivity and already-inspected valid data—not because an encoder failed an attribution ablation. Earlier Step27 uses of `M0` and immutable fields such as `no_transfer_capable_m0` retain only their historical local meanings. Authoritative definition: `docs/STEP7_STEP28_M0_ROLE_DEFINITION_20260724.zh.md`.
