...
- the formal freeze has closed output/self hashes, 23 explicitly hashed direct/transitive inputs, `model_scores_read=false`, `thresholds_lowered=false`, a reproducible `--check-only` result and `39/39` passing v8 contract tests. Contracts cover exact control isolation, mutually exclusive readiness slices, fixed-test identity, component disjointness, transitive provenance, English-train-only counting, the distinction between an absent v7 column, an imputable missing cell and a feature missing on every train row, and exact bridge-threshold binding across artifact/CSV/summary precision layers;
...
- local regression status after the threshold fix is `39/39` v8 contracts and `88/88` combined v6/v7/v8 contracts. The fix changes no labels, split, score, selected representation, model coefficient, threshold or metric; it only corrects cross-artifact validation;
