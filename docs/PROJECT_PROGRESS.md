...
`2026-07-17` the project moved to isolated branch `method/step24-content-independent-authorship` after Step21-Step23 established that local augmentation and additional item-distribution views add no independent controller evidence. Step24 is the only active method experiment. It does not generate seller, pair, item, text or identity labels. It replays the exact identifier-redacted Step15-v7 clean corpus and uses two frozen external multilingual style/authorship encoders: the EMNLP 2025 `Blablablab/multilingual-style-representation` model pinned at commit `b0147bbf450424fe72c8525fcc02e2e39e3a4024`, and the Findings ACL 2025 MIT-licensed `StyleDistance/mstyledistance` model pinned at `d66ed25e48225a503b21a65bc804caf06c886f96`. The preregistered pair view has exactly three cosines: identifier-redacted E5 semantics, multilingual authorship style and mStyleDistance. Fixed controls are raw cosines, E5-only LR/L2, style-only LR/L2 and the three-feature semantic-plus-style LR/L2 primary; there is no candidate search, class balancing, local encoder fine-tuning or valid/test selection.

Step24-v1 reads canonical train only: English source `401 = 116 positive / 285 negative` and Chinese target `573 = 229 positive / 344 negative`. The Chinese all-label OOF result is explicitly secondary internal-development evidence because `213/229` positives are train-only silver; non-silver and direct/component sensitivity slices are mandatory. Source-only transfer trains on English and scores Chinese train. Target adaptation uses five seller-component grouped folds, always retaining all English source rows and excluding the complete held-out Chinese components. Promotion requires at least `+0.03` target OOF AP and `+0.02` source-only AP, separate component-bootstrap lower bounds not below zero, no material source-only or target-OOF non-silver/direct-component degradation and no increase above `0.02` in template/topic negative score tails. Passing only authorizes one representative-valid check followed by a genuinely prospective Step20 holdout; failure freezes Step24 and redirects the paper to dataset/evidence-concept-drift/controlled-negative-results rather than further test-informed tuning.

Implementation is complete in `schema/step24_content_independent_authorship_policy.json`, `scripts/step24_*.py`, `scripts/run_step24_content_independent_authorship_linux_20260717.sh`, `tests/test_step24_content_independent_authorship_contracts.py` and `docs/STEP24_CONTENT_INDEPENDENT_AUTHORSHIP_PLAN_20260717.zh.md`. The Windows downloader pins both upstream commits and writes deterministic provenance; Linux rejects missing/mismatched provenance. Immutable outputs are isolated under `reports/step24_content_independent_authorship/v1_20260717/` and returned only as a complete SHA-256 manifest-bound directory. Python compilation, config-only entry points and nine read-only/pure contract tests pass on Windows; no Step24 embedding or model-performance computation has run on Windows. Numerical execution remains Linux-only.
