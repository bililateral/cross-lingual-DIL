...
下一步是设计 v8 修订：用独立事前固定、标签盲的公开编号全排列消除控制者结构与编号／输出顺序的耦合；将排版表面变化与控制者风格解耦；为失败回执加入所有硬门的小型诊断标量。不得删除不利探针、放宽阈值或枚举多个盐挑选通过数据。新实现经合同回归和科研复审后，只能生成新的设计级数据并重新质量审计；正式 500×4 和 M0/M1/M2/M3 继续禁止。
