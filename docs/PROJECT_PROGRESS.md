...
The implementation bundle is still internally closed only. A separate Git-parent receipt must pin commit A, its tree, and the four exact artifacts before external provenance is complete. Formal seeds, capabilities, candidates, rows, quality receipts, models, and metrics remain zero. This stage does not authorize a formal 500-world run, seed ceremony, dataset generation, or training.
