...
尝试 3 仅增加 `dict(...)` 将该内层只读映射转换为普通字典，并使用全新尝试编号、政策、私有目录与输出目录；科研输入、模型、参数、阈值和重抽样完全不变。真实深冻结政策进入实际固定模型拟合的直接反例 `1／1` 通过。依用户要求不再扩大回归范围；后续只提交修复、生成并单独提交新授权政策，然后唯一正式运行尝试 3。V9.4 有效终态产生前，1,004 世界方法资格数据和模型训练仍未授权。
