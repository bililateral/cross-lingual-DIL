...
- the formal freeze has closed output/self hashes, 23 explicitly hashed direct/transitive inputs, `model_scores_read=false`, `thresholds_lowered=false`, a reproducible `--check-only` result and `33/33` passing v8 contract tests. Contracts cover exact control isolation, mutually exclusive readiness slices, fixed-test identity, component disjointness, transitive provenance and English-train-only counting;
