...
`2026-09-03` V3 的主题／词汇诊断仍为曲线下面积 `0.769452`、平均精确率 `0.684380`，不能把完整文本模型的英文域内高分直接解释为身份能力。V3 只取得受控英文源训练资格。下一步冻结最小五臂：通用初始化、真实英文完整清洗文本、真实英文去词义视图、折内保持类数的多组固定英文标签置乱、语义／主题对照；各臂必须使用相同基座并进入完全相同的中文训练数据、优化预算和选择规则。去词义视图若用于主要归因，必须在英文和中文两端应用同一确定性转换；它最多证明“去除大部分词义后仍有可迁移的非词汇规律”，不能虚称纯作者身份。审核甲乙真值继续封存。
