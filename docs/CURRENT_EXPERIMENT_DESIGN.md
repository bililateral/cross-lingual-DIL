...
> 但主题／词汇诊断仍为 `0.769452`。因此当前只允许比较通用初始化、真实英文完整
> 文本、真实英文去词义视图、英文标签置乱和语义／主题对照，再让各臂进入相同中文
> 训练预算。主要证据是中文增量而非英文域内分数；去词义视图最多支持“非词汇规律
> 迁移”，不能独立证明纯作者身份。审核甲乙真值继续封存。正式数据、失败边界和散列
