...
import step28_v13_v1_13_method_root_quality_attempt3_v9_4 as quality
import step28_v13_v1_13_method_root_sealed_literal_scan_attempt3_v9_4 as sealed_scan
import step28_v13_v1_13_quality_text_probe_views_v9 as text_views


class MethodRootQualityAttempt3V94Contracts(unittest.TestCase):
    ...
    def test_counterfactual_second_replay_mutation_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(
            quality.MethodRootQualityAttempt3Error,
            "Counterfactual independent replay drift",
        ):
            quality.assert_counterfactual_replay(first, changed)

    def test_exact_public_replay_projects_endpoint_order_and_limits_both_splits(
        self,
    ) -> None:
        calls: list[tuple[str, ...]] = []

        def schedule(split: str) -> SimpleNamespace:
                ...
                public_worlds=[
                    {
                        ...
                        "noise_slot_by_seller_slot": [0, 1],
                    },
                    {
                        "world_ordinal": 1,
                        "world_uid": f"{split}-world-1",
                        "seller_uids": ["seller-left-1", "seller-right-1"],
                        "noise_slot_by_seller_slot": [1, 0],
                    },
                ],
                commitment={"split_schedule_commitment_sha256": split * 8},
            )

        def split_from_path(path: Path) -> str:
            return "train" if "train" in path.parts else "development"
        ...
        with (
            ...
            patch.object(quality.schedule_v94, "build_split_schedule", side_effect=schedule),
            patch.object(quality, "read_csv", side_effect=read_endpoints),
            patch.object(quality, "read_jsonl", side_effect=read_items),
            patch.object(
                quality.prebuild_v94,
                "build_truth_free_world_projection",
                return_value=[],
            ),
            patch.object(
                quality.replay_v94,
            ...
            result = quality.exact_v94_public_replay(
    ...
    def test_model_surface_world_excludes_controller_membership(self) -> None:
        public = {
            "world_ordinal": 0,
            "world_uid": "world-a",
            "seller_uids": [f"seller-{index:02d}" for index in range(28)],
            "noise_slot_by_seller_slot": list(range(28)),
        }
        schedule = SimpleNamespace(
            public_worlds=[public],
            controller_groups_by_world=[[public["seller_uids"]]],
        )
        with patch.object(
            quality.schedule_v94, "build_split_schedule", return_value=schedule
        ):
