...
class ScientificBuilderPolicyTests(unittest.TestCase):
    def test_policy_and_four_split_small_smoke_close(self) -> None:
        ...
        self.assertEqual(
            [row["split"] for row in context.world_records], list(scientific.SPLITS)
        )
...
class ScientificExactTitleEndpointTests(unittest.TestCase):
    ...
    def test_train_world_three_closes_the_observed_description_gap(self) -> None:
        record = next(
            row
    ...
    def test_all_104_preflight_worlds_have_qualified_clone_endpoints(self) -> None:
        relocated = 0
        row_count = 0
        ...
        self.assertEqual(row_count, 208)
