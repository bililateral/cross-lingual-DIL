#!/usr/bin/env python3
"""Run the current regression after the public-projection V1 implementation."""
...
import step28_v13_v1_13_regression_partition_v5 as previous


DEFAULT_CONTRACT = ROOT / "schema" / "step28_v13_v1_13_regression_partition_v6.json"
RESULT_VERSION = "2026-08-31-step28-v13-v1-13-regression-result-v6"
RESULT_MODE = "current_active_post_public_projection_v1_implementation"


class RegressionPartitionV6Error(RuntimeError):
    """Raised when the public-projection successor regression cannot close."""


def _exact_keys(value: Mapping[str, Any], expected: set[str], context: str) -> None:
    if set(value) != expected:
        raise RegressionPartitionV6Error(f"{context} schema drift")
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    try:
        ...
        raise RegressionPartitionV6Error("Cannot load V6 regression contract") from exc
    if not isinstance(contract, dict):
        raise RegressionPartitionV6Error("V6 regression contract must be an object")
    _exact_keys(
        contract,
        {
            "version",
    ...
    if contract["version"] != "2026-08-31-step28-v13-v1-13-regression-partition-v6":
        raise RegressionPartitionV6Error("V6 regression version drift")
    if contract["status"] != "FROZEN_POST_PUBLIC_PROJECTION_V1_IMPLEMENTATION_REGRESSION":
        raise RegressionPartitionV6Error("V6 regression status drift")
    if contract["training_authorized"] is not False or contract["audit_truth_authorized"] is not False:
        raise RegressionPartitionV6Error("V6 regression cannot authorize training or audit truth")
    unsigned = dict(contract)
    observed = unsigned.pop("canonical_self_hash")
    if base.sha256_bytes(base.canonical_json_bytes(unsigned)) != observed:
        raise RegressionPartitionV6Error("V6 regression self hash drift")
    for key in (
        "base_contract_pin",
        "previous_contract_pin",
        "previous_runner_pin",
    ...
    if not isinstance(superseded, list) or len(superseded) != 4 or len(set(superseded)) != 4:
        raise RegressionPartitionV6Error("Exactly four predecessor discovery tests must be partitioned")
    _exact_keys(
...
def _load_json_pin(pin: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RegressionPartitionV6Error("Pinned result must be an object")
    return value
...
def validate_predecessor_outcomes(contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if ids[:3] != previous_contract["superseded_discovery_test_ids"]:
        raise RegressionPartitionV6Error("Inherited V2/V3/V4 discovery IDs drift")
    ...
    if (
        ...
        raise RegressionPartitionV6Error("V5 discovery predecessor did not pass")
    return {
        **inherited,
        "v5_test_id": v5_id,
        "v5_outcome": "success_in_v5_result",
        "v5_result_sha256": contract["previous_result_pin"]["sha256"],
    }
...
def load_current_suite(
    ...
    if observed_superseded != superseded_ids:
        raise RegressionPartitionV6Error(
    ...
    if any(observed != expected for observed, expected in checks):
        raise RegressionPartitionV6Error("Public-projection discovery boundary drift")
...
def run_current(contract_path: Path, output: Path | None) -> int:
    ...
    if (
        ...
        raise RegressionPartitionV6Error("Fixture-unstarted boundary drift")
    ...
    if payload["unstarted_test_ids"] != expected_unstarted:
        raise RegressionPartitionV6Error("Current fixture-skip boundary drift")
    if payload["tests_run"] != len(active_ids) - len(expected_unstarted):
        raise RegressionPartitionV6Error("Current active test count did not close")
    ...
    if (
        ...
        raise RegressionPartitionV6Error("Current documented skip events drift")
    previous_contract = previous.load_contract(
        ROOT / contract["previous_contract_pin"]["path"]
    )
    payload["active_scheduled_test_count"] = len(active_ids)
    payload["historical_result_sha256"] = previous_contract[
...
def main() -> None:
    ...
    try:
        ...
        print(f"Public-projection successor regression failed: {exc}", file=sys.stderr)
