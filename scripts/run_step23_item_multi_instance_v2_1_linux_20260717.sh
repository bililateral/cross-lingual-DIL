...
OUTPUT_ROOT="reports/step23_item_multi_instance/v2_1_20260717"

echo "[1/8] Validate Step22 freeze and corrected Step23-v2.1 contracts"
...
echo "[3/8] Build the corrected label-blind identifier-redacted real train-item cache"
"$PYTHON_BIN" scripts/step23_build_item_text_cache.py \
  --policy "$POLICY"
...
echo "[7/8] Verify corrected redaction, fixed-primary, and train-only artifact scope"
"$PYTHON_BIN" -c 'import json,pathlib; root=pathlib.Path("reports/step23_item_multi_instance/v2_1_20260717"); item=json.loads((root/"item_selection_summary.json").read_text(encoding="utf-8")); result=json.loads((root/"step23_grouped_oof_evaluation.json").read_text(encoding="utf-8")); artifacts=json.loads((root/"step23_model_artifacts.json").read_text(encoding="utf-8")); raw_total=sum(int(v["raw_item_count"]) for v in item["selection_stats"].values()); assert int(item["redaction_diagnostics"]["exact_overlap_disabled_item_count"]) < raw_total; assert item["valid_test_items_encoded"] is False and item["synthetic_item_count"] == 0; assert result["valid_or_test_scores_used"] is False and result["publication_holdout_untouched"] is True; assert result["preregistration"]["candidate_selection_performed"] is False; assert result["preregistration"]["primary_model"] == "aggregate_plus_distribution_primary"; assert artifacts["primary_model"] == result["preregistration"]["primary_model"]; print({"status":"scope_verified","selected_items":item["selected_item_count"],"exact_overlap_disabled":item["redaction_diagnostics"]["exact_overlap_disabled_item_count"],"promotion_eligible":result["promotion"]["eligible"]})'

echo "[8/8] Build the complete content-addressed synchronization manifest"
"$PYTHON_BIN" scripts/step23_build_sync_manifest.py \
  --policy "$POLICY"

echo "Step23-v2.1 completed. Outputs: $OUTPUT_ROOT/"
