#!/usr/bin/env python3
"""Frozen metric core for the V9.3 shortcut-quality audit."""
...
VERSION = "2026-08-26-step28-v13-v1-13-quality-probe-core-v9-3"


class QualityProbeCoreV93Error(common.ContractError):
...
def freeze_matrix(
    *,
    ...
    if type(take_ownership) is not bool:
        raise QualityProbeCoreV93Error("Matrix ownership flag drift")
    if take_ownership:
    ...
    if (
        ...
        raise QualityProbeCoreV93Error(f"Frozen matrix closure drift: {view}")
    commitment = {
        "version": VERSION,
        "view": view,
...
def _validate_family(
    ...
    if not views or tuple(development) != views:
        raise QualityProbeCoreV93Error("Probe family view order drift")
    ...
    for view in views:
        if (
            ...
            raise QualityProbeCoreV93Error(f"Probe family matrix drift: {view}")
    return views
...
def _validate_labels(labels: np.ndarray, expected_rows: int, *, label: str) -> np.ndarray:
    ...
    if (
        ...
        raise QualityProbeCoreV93Error(f"{label} label closure drift")
...
def symmetric_auc(labels: np.ndarray, scores: np.ndarray) -> float:
    if labels.shape != scores.shape or not np.isfinite(scores).all():
        raise QualityProbeCoreV93Error("Symmetric AUC input drift")
    ...
    *,
    ...
    with threadpool_limits(limits=1):
        ...
        if int(logistic.n_iter_[0]) >= int(logistic_config["max_iter"]):
            raise QualityProbeCoreV93Error("Quality logistic reached max_iter")
    ...
    if any(not np.isfinite(value).all() for value in finite_objects):
        raise QualityProbeCoreV93Error("Probe fit produced nonfinite state")
    return {
...
def generate_bootstrap_draws(
    *, replicates: int, world_count: int, seed: int
    ...
    if any(type(value) is not int or value <= 0 for value in (replicates, world_count, seed)):
        raise QualityProbeCoreV93Error("Bootstrap constants are invalid")
...
def _bootstrap_family_upper(
    *,
    ...
    if (
        ...
        raise QualityProbeCoreV93Error("Bootstrap input contract drift")
    ...
    for model_name in sorted(score_family, key=lambda value: value.encode("utf-8")):
        ...
        if scores.shape != labels.shape or not np.isfinite(scores).all():
            raise QualityProbeCoreV93Error("Bootstrap score vector drift")
        ...
        for start in range(0, len(draws), batch_size):
            ...
            if np.any(total_positive <= 0) or np.any(total_negative <= 0):
                raise QualityProbeCoreV93Error("Bootstrap replicate lost a class")
            auc = np.sum(
    ...
    if not np.isfinite(family_auc).all() or not np.isfinite(family_ap).all():
        raise QualityProbeCoreV93Error("Bootstrap family maxima are nonfinite")
    return {
...
def _single_feature_maximum(matrix: FrozenMatrix, labels: np.ndarray) -> float:
    ...
    if not math.isfinite(best):
        raise QualityProbeCoreV93Error("Single-feature scan is nonfinite")
    return float(best)
...
def evaluate_family(
    *,
    ...
    if not 0.0 < average_precision_baseline < 1.0:
        raise QualityProbeCoreV93Error("Average-precision baseline drift")
    ...
    for view in views:
        ...
        for model, score in scores.items():
            ...
            if score.shape != development_y.shape or not np.isfinite(score).all():
                raise QualityProbeCoreV93Error("Probe score vector drift")
    ...
    if bootstrap:
        ...
        draws = generate_bootstrap_draws(
            ...
            world_count=int(bootstrap_policy["development_world_count"]),
        ...
        if observed_draw_hash != bootstrap_policy["draws_raw_i8_c_sha256"]:
            raise QualityProbeCoreV93Error("Bootstrap draw commitment drift")
        ...
        output["bootstrap"] = _bootstrap_family_upper(
            ...
            draws=draws,
            batch_size=int(bootstrap_policy["streaming_batch_size"]),
        )
    return output
