#!/usr/bin/env python3
"""Run the current regression after the English-151 replay-v2 milestone."""
...
import step28_v13_v1_13_regression_partition_v3 as previous


DEFAULT_CONTRACT = ROOT / "schema" / "step28_v13_v1_13_regression_partition_v4.json"
RESULT_VERSION = "2026-08-31-step28-v13-v1-13-regression-result-v4"
RESULT_MODE = "current_active_post_english_151_replay_v2"


class RegressionPartitionV4Error(RuntimeError):
    """Raised when the current successor regression cannot be reproduced."""


def _exact_keys(value: Mapping[str, Any], expected: set[str], context: str) -> None:
    if set(value) != expected:
        raise RegressionPartitionV4Error(f"{context} schema drift")
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    try:
        ...
        raise RegressionPartitionV4Error("Cannot load V4 regression contract") from exc
    if not isinstance(contract, dict):
        raise RegressionPartitionV4Error("V4 regression contract must be an object")
    _exact_keys(
        contract,
        {
            "version",
        ...
        "V4 regression contract",
    )
    if contract["version"] != "2026-08-31-step28-v13-v1-13-regression-partition-v4":
        raise RegressionPartitionV4Error("V4 regression version drift")
    if contract["status"] != "FROZEN_POST_ENGLISH_151_REPLAY_V2_REGRESSION":
        raise RegressionPartitionV4Error("V4 regression status drift")
    if contract["training_authorized"] is not False:
        raise RegressionPartitionV4Error("V4 regression cannot authorize training")
    if contract["audit_truth_authorized"] is not False:
        raise RegressionPartitionV4Error("V4 regression cannot open audit truth")
    unsigned = dict(contract)
    observed = unsigned.pop("canonical_self_hash")
    if base.sha256_bytes(base.canonical_json_bytes(unsigned)) != observed:
        raise RegressionPartitionV4Error("V4 regression self hash drift")
    for key in (
        "base_contract_pin",
        "base_runner_pin",
        "previous_contract_pin",
        ...
        if not isinstance(pin, dict):
            raise RegressionPartitionV4Error(f"{key} must be an object")
    ...
    if (
        ...
        or len(superseded) != 2
        or len(set(superseded)) != 2
        or any(not isinstance(value, str) or not value for value in superseded)
    ):
        raise RegressionPartitionV4Error(
            "Exactly two predecessor discovery tests must be partitioned"
        )
    _exact_keys(
...
def _load_json_pin(pin: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise RegressionPartitionV4Error("Pinned result cannot be decoded") from exc
    if not isinstance(value, dict):
        raise RegressionPartitionV4Error("Pinned result must be an object")
    return value


def validate_predecessor_outcomes(contract: Mapping[str, Any]) -> dict[str, Any]:
    """Prove both discovery-only tests passed before their boundaries became stale."""
    previous_contract = previous.load_contract(
        ROOT / contract["previous_contract_pin"]["path"]
    )
    v2_evidence = previous.validate_predecessor_outcome(previous_contract)
    ids = contract["superseded_discovery_test_ids"]
    if ids[0] != v2_evidence["test_id"]:
        raise RegressionPartitionV4Error("V2 discovery predecessor ID drift")
    ...
    if (
        ...
        raise RegressionPartitionV4Error("V3 discovery predecessor did not pass")
    return {
        "v2_test_id": ids[0],
        "v2_outcome": "success_in_v2_result",
        "v3_test_id": v3_id,
        "v3_outcome": "success_in_v3_result",
        "v3_result_sha256": contract["previous_result_pin"]["sha256"],
    }
...
def load_current_suite(
    ...
    if any(observed != expected for observed, expected in checks):
        raise RegressionPartitionV4Error("English-151 discovery boundary drift")
    return (
...
def run_current(contract_path: Path, output: Path | None) -> int:
    ...
    if (
        ...
        raise RegressionPartitionV4Error("Fixture-unstarted boundary drift")
    ...
    if payload["unstarted_test_ids"] != expected_unstarted:
        raise RegressionPartitionV4Error("Current fixture-skip boundary drift")
    if payload["tests_run"] != len(active_ids) - len(expected_unstarted):
        raise RegressionPartitionV4Error("Current active test count did not close")
    ...
    if (
        ...
        raise RegressionPartitionV4Error(
            "Current documented skip events drift: "
            f"count={len(payload['skipped'])} sha256={payload['skip_events_sha256']}"
        )
    payload["active_scheduled_test_count"] = len(active_ids)
    payload["historical_result_sha256"] = contract["historical_result_pin"][
        "sha256"
    ]
...
def main() -> None:
    ...
    try:
        ...
        RegressionPartitionV4Error,
        previous.RegressionPartitionV3Error,
        previous.previous.RegressionPartitionV2Error,
        base.RegressionPartitionError,
        OSError,
    ) as exc:
        print(f"English-151 successor regression failed: {exc}", file=sys.stderr)
